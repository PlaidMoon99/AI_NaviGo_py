...
class PlacesHelper:
    ...
    async def get_place_details(self, place_id: str) -> Optional[PlaceDetailsModel]:
        ...
        params = {
            "place_id": place_id,
            "fields": "name,formatted_address,geometry,opening_hours,rating,reviews,price_level",
            "language": "ko",
            "key": self.api_key
        }
    ...
    async def get_place_photos_batch(self, place_names: List[str]) -> Dict[str, str]:
        """여러 장소의 사진을 한 번에 가져옵니다."""
        photos = {}
        
        async with aiohttp.ClientSession() as session:
            for place_name in place_names:
                try:
                    # 1. 장소 검색으로 place_id 획득
                    ...
                    async with session.get(search_url, params=search_params) as response:
                        ...
                        # 2. 장소 상세 정보에서 photo reference 획득
                        ...
                        async with session.get(details_url, params=details_params) as details_response:
                            ...
                            if (details_data.get("status") == "OK" and 
                                ...
                                photos[place_name] = photo_url
                
                except Exception as e:
                    print(f"Error fetching photo for {place_name}: {str(e)}")
                    continue
